import { getActiveSaleByCoupanCode } from "@/sanity/lib/sales/getActiveSaleByCoupanCode";
async function ChristmasBanner () {
    const sale = await getActiveSaleByCoupanCode("MRIDUL50")
    if(!sale?.isActive) {
                    ...
                    <div className="flex">
                        <div className="bg-white text-black py-4 px-6 rounded-full shadow-md transform hover:scale-105 transition duration-300">
                            <span className="font-bold text-base sm:text-xl">
                                UseCode : {" "}
                                <span className="text-red-600">{sale.coupanCode}</span>
